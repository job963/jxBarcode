...
class MyOrder extends MyOrder_parent
{
    ...
    public function genPdf( $sFilename, $iSelLang = 0 )
    {
		...
		// adding barcode with invno or delno to pdf
    ...
    protected function _setOrderArticlesToPdf( $oPdf, &$iStartPos, $blShowPrice = true )
    {
        ...
        foreach ( $this->_oArticles as $key => $oOrderArt ) {
			...
			$this->_sFont = 'times';
			//$oPdf->setFont( $oPdfBlock->getFont(), '', 8 );
			$oPdf->setFont( 'robotocondensed', '', 10 );
	...
	public function pdfBarcode( $oPdf )
	{
		...
		// For Code 39
		//$cBcText = '*' . (1000+$this->oxorder__oxbillnr->value) . '*';
		$cBcText = (string) (1000+$this->oxorder__oxbillnr->value);

		$oPdf->write1DBarcode( $cBcText, 'C128B', '', '62', '', 5, 0.4, $jxBcStyle, 'T' );
		$oPdf->Ln();
	}
}
